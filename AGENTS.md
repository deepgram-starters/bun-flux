...
Bun demo app for Deepgram Flux.
...
| `DEEPGRAM_BASE_URL` | No | production endpoint | WebSocket endpoint override for non-production Flux environments, such as `wss://api.staging.deepgram.com` |
...
```
feat(bun-flux): add diarization support
fix(bun-flux): resolve WebSocket close handling
refactor(bun-flux): simplify session endpoint
